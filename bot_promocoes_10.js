...
const OUTPUT_FILE = 'promocoes_10.json';
...
async function runPromoBot() {
    try {
        console.log('\n');
        console.log('╔═══════════════════════════════════════════════════════════════╗');
        console.log('║           💰 BOT CAÇADOR DE PROMOÇÕES (< R$ 10)              ║');
        console.log('╚═══════════════════════════════════════════════════════════════╝');
        ...
        for (let i = 0; i < productUrls.length; i += BATCH_SIZE) {
            ...
            try {
                ...
                for (const data of batchResults) {
                    ...
                    let found = false;
                    let selectedPrice = '';
                    let selectedPriceValue = Infinity;
                    let type = '';

                    // Verifica se Primária atende ao critério
                    if (primStock && primPrice < MAX_PRICE) {
                        found = true;
                        selectedPrice = data.primary.price;
                        selectedPriceValue = primPrice;
                        type = 'Primária';
                    }

                    // Verifica se Secundária atende ao critério (e se é mais barata que a primária se ambas existirem)
                    if (secStock && secPrice < MAX_PRICE) {
                        if (!found || secPrice < selectedPriceValue) {
                            found = true;
                            selectedPrice = data.secondary.price;
                            selectedPriceValue = secPrice;
                            type = 'Secundária';
                        }
                    }
                    ...
                    if (found) {
                        ...
                        console.log(`\n✨ ENCONTRADO: ${productName} - ${selectedPrice} (${type})`);
