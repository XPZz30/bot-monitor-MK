...
const MAX_PRICE = 30.00;
const OUTPUT_FILE = 'promocoes_30.json';
...
async function runPromoBot() {
    try {
        console.log('\n');
        console.log('╔═══════════════════════════════════════════════════════════════╗');
        console.log('║           💰 BOT CAÇADOR DE PROMOÇÕES (<= R$ 30)             ║');
        console.log('╚═══════════════════════════════════════════════════════════════╝');
        ...
        for (let i = 0; i < productUrls.length; i += BATCH_SIZE) {
            ...
            try {
                ...
                // 3️⃣ Filtra os resultados
                let newPromotionsCount = 0;
                for (const data of batchResults) {
                    if (!data) continue;

                    const secPrice = parsePrice(data.secondary.price);
                    const secStock = data.secondary.stock;

                    // Critério: Preço Secundária <= 30 E Estoque Disponível
                    if (secStock && secPrice <= MAX_PRICE) {
                        const productName = extractProductName(data.url);
                        console.log(`\n✨ ENCONTRADO: ${productName} - ${data.secondary.price}`);

                        const isExpensive = secPrice > 20.00;

                        const promoItem = {
                            name: productName,
                            url: data.url,
                            image: data.image,
                            price: data.secondary.price,
                            priceValue: secPrice, // Valor numérico para ordenação se necessário
                            stock: true,
                            capturedAt: new Date().toISOString()
                        };

                        // Adiciona a especificação e o identificador exclusivo se for maior que 20 reais
                        if (isExpensive) {
                            promoItem.isExpensive = true;
                            promoItem.category = "mais_caro";
                            // Gera um id estático único usando randomBytes
                            promoItem.exclusiveId = `EXPENSIVE-${crypto.randomBytes(4).toString('hex').toUpperCase()}`;
                        }
